...
export const CatalogPicker: React.FC<CatalogPickerProps> = ({
  ...
  useEffect(() => {
    Promise.all([
      api.catalogItems.list(),
      api.scopeBlocks.list(),
      api.textBlocks.list(),
    ])
      .then(([i, s, t]) => {
        setItems(i);
  ...
  return (
    <Modal
      ...
      icon={<Boxes className="w-5 h-5 text-primary-green" />}
      onClose={onClose}
        ...
        <div className="space-y-2">
          ...
          {items.map((it) => (
            <Row key={it.id} className={card} onAdd={() => onInsertItem(it)}>
              ...
              <span className="text-[11px] font-bold text-primary-green ml-2 shrink-0">
...
const Empty = () => (
  <p className="text-xs text-slate-400 py-6 text-center">Catálogo vacío.</p>
);
...
}> = ({ className, onAdd, children }) => (
  <div
    className={`flex items-center justify-between gap-2 p-3 border rounded-xl ${className}`}
  >
    <div className="flex items-center justify-between gap-2 flex-grow min-w-0">
      {children}
    </div>
    <button
      onClick={onAdd}
      title="Agregar a la cotización"
      className="p-1.5 bg-primary-green/10 text-primary-green rounded-xl hover:bg-primary-green/20 transition-colors shrink-0"
    >
