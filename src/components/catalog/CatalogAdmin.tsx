...
export const CatalogAdmin: React.FC<CatalogAdminProps> = ({
  ...
  const input = `border rounded-xl p-2 text-xs outline-none focus:ring-2 focus:ring-primary-green ${
    isDarkMode
  ...
  const AddBtn: React.FC<{ onClick: () => void; disabled?: boolean }> = ({
    ...
    <button
      onClick={onClick}
      disabled={disabled}
      className="px-3 py-2 rounded-xl bg-primary-green text-white text-xs font-bold flex items-center gap-1.5 disabled:opacity-40 hover:brightness-110 transition-all"
    >
  ...
  return (
    <Modal
      ...
      icon={<Settings2 className="w-5 h-5 text-primary-green" />}
      onClose={onClose}
      ...
      {tab === "items" && (
        <div className="space-y-3">
          ...
          {items.map((it) => (
            <div
              key={it.id}
              className={`flex items-center gap-2 p-3 border rounded-xl ${card}`}
            >
              <span className="text-xs whitespace-pre-line flex-grow">
                {it.description}
              </span>
              ...
              <DelBtn
                onClick={async () => {
                  await api.catalogItems.remove(it.id);
                  await reloadItems();
                }}
              />
            </div>
          ...
          <div className="flex flex-col gap-2 p-3 border rounded-xl border-dashed">
            ...
            <div className="self-end">
              <AddBtn
                ...
                onClick={async () => {
                  const list = newScopeItems
                    .split("\n")
                    .map((s) => s.trim())
                    .filter(Boolean);
                  await api.scopeBlocks.create({
                    title: newTitle,
                    items: list,
                  });
                  setNewTitle("");
            ...
            >
              <div className="flex-grow">
                ...
                <p className="text-[10px] text-slate-400">
                  {s.items.length} elementos
                </p>
              </div>
              <DelBtn
                onClick={async () => {
                  await api.scopeBlocks.remove(s.id);
                  await reloadScope();
                }}
              />
            </div>
          ...
          {text.map((t) => (
            ...
            >
              <div className="flex-grow min-w-0">
                <span className="text-[9px] font-bold uppercase tracking-wider text-primary-green">
                  {TEXT_TYPE_LABELS[t.type]}
                </span>
                <p className="text-xs truncate">{t.label}</p>
                {t.value && (
                  <p className="text-[10px] text-slate-400">{t.value}</p>
                )}
              </div>
              <DelBtn
                onClick={async () => {
                  await api.textBlocks.remove(t.id);
                  await reloadText();
                }}
              />
            </div>
